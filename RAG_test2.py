...
data = [
    ...
    },
    {
        ...
        "content": "Margaret is 32 years old."
    },
    {
...
data_embeddings = []
for item in data:
    combined_text = item["topic"] + ". " + item["content"]
    embedding = ST.encode(combined_text, convert_to_tensor=True)
    data_embeddings.append(embedding)

def find_most_relevant_content(prompt, data, data_embeddings, model):
    prompt_embedding = model.encode(prompt, convert_to_tensor=True)
    cosine_scores = util.cos_sim(prompt_embedding, torch.stack(data_embeddings))
    
    most_relevant_idx = torch.argmax(cosine_scores).item()
    
    return data[most_relevant_idx]

prompt = "Tell me about Margaret's appearance."

most_relevant_content = find_most_relevant_content(prompt, data, data_embeddings, ST)
